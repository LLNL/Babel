...
smartCp(struct sidl_fcomplex__array *a) {
  sidl_fcomplex__array_addRef(a);
  return a;
}
...
sidl_fcomplex__array_cborrow_f03(struct sidl_fcomplex *firstElement,
                           int32_t dimen,
                           int32_t lower[],
                           int32_t upper[],
                           int32_t stride[])
{
  struct sidl_fcomplex__array *a = 
    sidl_fcomplex__array_borrow(firstElement, dimen, lower, upper, stride);
  if (a) {
...
void
SIDLFortran90Symbol(sidl_fcomplex__array_bindc_cast_c,
                    SIDL_FCOMPLEX__ARRAY_BINDC_CAST_C,
                    sidl_fcomplex__array_bindc_cast_c)
  (struct sidl__array **array, int32_t *dimen, struct sidl_fortran2003_array *result)
{
  struct sidl_fcomplex__array *tmp = sidl_fcomplex__array_cast(*array);
  if(tmp && sidlArrayDim(tmp) != *dimen) tmp = NULL;
  (void) sidl_fcomplex__array_convert2f03(tmp, *dimen, result);
}

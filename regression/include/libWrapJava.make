...
libClient.scl :  $(STUBSRCS) $(client_STUBSRCS)
	rm -f libClient.scl
	...
	grep __register $(STUBSRCS) $(client_STUBSRCS) /dev/null | awk 'BEGIN {FS=":"} { print $$1}' | sort -u | sed -e 's/_jniStub.c//g' -e 's/_/./g' | awk ' { printf "    <class name=\"%s\" desc=\"java\" />\n", $$1 }' >> libClient.scl
...
clean-local:
	...
	if test "X$(srcdir)" != "X."; then		   \
	  rm -f $(IMPLSRCS) $(IMPLHDRS) babel.make client_babel.make;   \
	  rm -rf `find . -type d ! \( -name CVS -o -name .svn -o -name "." -o -name .libs -o -name .deps \) `;  \
	else						   \
	  if test -d $(JAVAPKG); then \
	    for file in $(STUBJAVA) $(client_STUBJAVA); do	   \
	      rm -f $(JAVAPKG)/$$file;			   \
	    done;						   \
	    rm -f $(JAVAPKG)/*.class;			   \
	  fi;					    	   \
...
installcheck-local:
	...
	$(MAKE) $(AM_MAKEFLAGS)				\
	  ...
	  $(check_LTLIBRARIES) java-stamp $(SCLFILES)

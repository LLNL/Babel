...
subroutine testneg(fib, test)
  ...
  if (is_null(iExcept)) then
     ...
     call isType(iExcept, &
          'Exceptions.NegativeValueException', isone, throwaway)
...
subroutine testdeep(fib, test)
  ...
  if (is_null(iExcept)) then
     ...
     call isType(iExcept, 'Exceptions.TooDeepException', &
          isone, throwaway)
     if (isone .eqv. .true.) then
        call reporttest(.true., test)
     else
...
subroutine testbig(fib, test)
  ...
  if (is_null(iExcept)) then
     ...
     call isType(iExcept, 'Exceptions.TooBigException', &
          isone, throwaway)
     if (isone .eqv. .true.) then
        call reporttest(.true., test)
     else
...
program exceptionstest
  ...
  if ( remoteURL .ne. '' ) then
     ...
     call addProtocol('simhandle', 'sidlx.rmi.SimHandle', retval, throwaway)

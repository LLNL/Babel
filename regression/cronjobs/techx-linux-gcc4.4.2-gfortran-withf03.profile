...
export PROFILE_NAME="techx-linux-gcc4.4.2-${FORTRAN}-withf03.profile"
export TESTGID=research
export REPLYTO="muszala@txcorp.com"
...
export PATH=/usr/lib64/qt-3.3/bin:/usr/kerberos/sbin:/usr/kerberos/bin:/usr/lib64/ccache:/usr/local/bin:/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/sbin:/usr/NX/bin:/usr/local/contrib/jdk1.6.0_16/bin
export LD_LIBRARY_PATH=
export JNI_INCLUDES=$JAVAHOME/include

export SVN=svn
if test `whoami` == "muszala"; then
 SVNUSER=muszala@
else
 SVNUSER=
fi
...
export PACKAGING_BUILDDIR=/scr_dijkstra/muszala/babel_scratch/nightly-run
...
case `hostname -s` in
    "dijkstra")
	CONFIG_FLAGS="--enable-java=/usr/local/contrib/jdk1.6.0_16 JAR=/usr/local/contrib/jdk1.6.0_16/bin/jar"
;;
    "iter")
	CONFIG_FLAGS="JAVAPREFIX=/usr/lib/jvm/java-1.6.0"
;;
    "multipole")
	CONFIG_FLAGS="JAVAPREFIX=/usr/java/jdk1.6.0_07"
;;
esac

export CONFIG_FLAGS="$CONFIG_FLAGS --enable-fortran03=yes"

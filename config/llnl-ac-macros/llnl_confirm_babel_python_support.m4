...
AC_DEFUN([LLNL_CONFIRM_BABEL_PYTHON_SUPPORT], [
  ...
  if test "X$enable_python" != "Xno"; then
    ...
    if test "X$enable_shared" = "Xno"; then
      enable_python=no
      AC_MSG_WARN([Python configuration failed: shared libraries disabled!])
      msgs="$msgs
          Python support disabled against request: shared libs disabled"
    else
      if test "X$llnl_cv_python_numerical" != "Xyes"; then
        enable_python=no
        AC_MSG_WARN([Python configuration failed: missing Numeric Python!])
	msgs="$msgs
          Python support disabled against request: missing Numeric Python"
      else
        if test "X$XML2_CONFIG" = "Xno"; then
	  enable_python=no
	  AC_MSG_WARN([Python configuration failed: missing/old libxml2!])
	  msgs="$msgs
          Python support disabled against request: missing/old libxml2"
	else
	  if test "X$llnl_python_shared_library_found" != "Xyes"; then
            AC_MSG_WARN([Python shared library missing.  Support for server-side Python disabled!])
            msgs="$msgs

 ...
 * Release:       $Name:  $
 * Revision:      @(#) $Id: sidl_SIDLException_Impl.c 7465 2012-05-02 20:05:00Z adrian $
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  if (data) 
  {
    ...
    /* It's a questionable descision to put the memory exception here.
       I'm putting it in because a memory exception will give more info
       than a normal SIDLException in out of memory condition. -Jim */
    ...
    sidl_MemAllocException_add(ex, __FILE__, __LINE__, "sidl.SIDLException._ctor", _ex);
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  /* No memory exception, correct behavior if no mem is to do nothing.*/
  char *result = 
    ((data && data->d_message) 
  ...
  *_ex = 0;
  ...
  if(!data) {
    data = (struct sidl_SIDLException__data *)
      malloc(sizeof (struct sidl_SIDLException__data));
    if(!data) {
      /* An out of memory error is probably more informative than what was thrown here. -Jim */
      sidl_MemAllocException ex = sidl_MemAllocException_getSingletonException(_ex);
      sidl_MemAllocException_setNote(ex, "Out of memory.", _ex);
      sidl_MemAllocException_add(ex, __FILE__, __LINE__, "sidl.SIDLException.unpackObj", _ex);
      *_ex = (sidl_BaseInterface)ex;
      goto EXIT;
    }
    sidl_SIDLException__set_data(self,data);

 ...
 * Date:        $Date: 2012-10-26 10:21:39 -0700 (Fri, 26 Oct 2012) $
...
static void sidl_Java_getJVM(void)
{
  ...
  if (s_jvm == NULL) {
...
#endif
    /*This section of code takes any flags passed in through the environment
    ...
    /*
       Use this option to disable the JIT
       options[0].optionString = "-Djava.compiler=NONE";
    */
    options[0].optionString = clspath;
    /*
       FIXME: this assumption will presumably break the RMI/Cygwin regression tests
    */
    options[1].optionString = "-Dsidl.library.name=sidl-" VERSION;
    ...
    if (!jvmf) { /* not in main: */
      ...
      if (url) {
        ...
        if (dll) {
          ...
          sidl_DLL_deleteRef(dll,&_ex); SIDL_CHECK(_ex);
	  if (jvmf == NULL) {
	    fprintf(stderr, "Babel: Unable to initialize Java Virtual Machine\n\
 Babel: Symbol \"JNI_CreateJavaVM\" not found in %s.\n", url);
	  }
        }
					     ...
					     jobject obj)
{
  ...
  (*env)->SetLongField(env, obj, s_ior_field, (jlong)(ptrdiff_t) NULL);
}
  ...
  if (ex != NULL) {
	...
	if (obj != NULL) {
	  if((*env)->Throw(env, obj) != 0)
	    fprintf(stderr, "Babel: Unable rethrow the exception received.\n");
	}
	break;
      }
    }
  ...
  jstring str)
{
  if (str != (jstring) NULL) {
    jclass e = (*env)->FindClass(env, "java/lang/InternalError");
    if (e != NULL) {
      (*env)->ThrowNew(env, e, (*env)->GetStringUTFChars(env, str, NULL));
      (*env)->DeleteLocalRef(env, e);
    }
  }
  return;

 ...
 * Release:       $Name:  $
 * Revision:      @(#) $Id: sidl_rmi_InstanceRegistry_Impl.c 7465 2012-05-02 20:05:00Z adrian $
...
objecthash(void *ky)
{
  register ptrdiff_t val = (ptrdiff_t)ky;
#if SIZEOF_PTRDIFF_T == 2*SIZEOF_INT
  val ^= (val >> (SIZEOF_INT * 8));
#elif SIZEOF_PTRDIFF_T > SIZEOF_INT
  register ptrdiff_t val2 = val >> (SIZEOF_INT * 8);
  val ^= val2;
  while (val2 > 0) {
    val2 >>= (SIZEOF_INT * 8);
    val ^= val2;
...
static void
rmi_InstanceRegistry_cleanup(void* ignore)
{
#ifdef HAVE_PTHREAD
     (void)pthread_mutex_lock(&(s_counter_mutex));
#endif /* HAVE_PTHREAD */
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  /* Since this should really only happen once, before any threads have started do I need mutexs?*/
     ...
     (void)pthread_mutex_lock(&(s_counter_mutex));
#endif /* HAVE_PTHREAD */
  s_counter = (char*)sidl_String_alloc(INITIAL_SIZE);
  memset(s_counter, '0', INITIAL_SIZE);
  s_counter[INITIAL_SIZE] = '\0';
#ifdef HAVE_PTHREAD
     (void)pthread_mutex_unlock(&(s_counter_mutex));
#endif /* HAVE_PTHREAD */
  ...
  (void)sidl_atexit(rmi_InstanceRegistry_cleanup, NULL);

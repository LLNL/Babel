 ...
 * Release:       $Name:  $
 * Revision:      @(#) $Id: sidl_DLL_Impl.c 7465 2012-05-02 20:05:00Z adrian $
  ...
  /* out */ sidl_BaseInterface *_ex)
{
...
#if defined(PIC) || !defined(SIDL_PURE_STATIC_RUNTIME)
  /* loadLazy not supported in move to libtool 2.2.2 */
  /* handle = lt_dlopen(dllfile, loadGlobally, loadLazy); */
 {
   ...
   handle = lt_dlopenadvise(dllfile, advise);
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  if (data->d_library_handle) {
    ...
    if (s_sidl_debug_dlopen && !address) {
      const char *errmsg = lt_dlerror();
      fprintf(stderr, "babel: Looking up symbol %s failed in library %s: %s\n", linker_name, 
	      data->d_library_name, errmsg);
    }
#endif
  }
  return address;
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  if (data->d_library_handle) {
    char* linker = sidl_String_concat2(sidl_name, "__createObject");
    char* ptr    = linker;
    ...
    if (s_sidl_debug_dlopen && !ctor) {
      const char *errmsg = lt_dlerror();
      fprintf(stderr, "babel: Looking up constructor symbol %s failed in library %s: %s\n", linker, 
	      data->d_library_name, errmsg);
    }
#else
    ctor = NULL;

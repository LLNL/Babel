 ...
 * Release:       $Name:  $
 * Revision:      @(#) $Id: sidl_BaseClass_Impl.c 7465 2012-05-02 20:05:00Z adrian $
  ...
  *_ex = 0;
  {
    ...
    /* Preload the MemAllocException */
    sidl_MemAllocException m_ex = 
      sidl_MemAllocException_getSingletonException(_ex);
    sidl_MemAllocException_deleteRef(m_ex, _ex);
  ...
  /* out */ sidl_BaseInterface *_ex)
{
  *_ex = 0;
  ...
  if(!data) {
    ...
    sidl_MemAllocException_add(ex, __FILE__, __LINE__, "BaseClass__ctor", _ex);
  ...
  *_ex = 0;
  ...
  int32_t newRefcount;
   ...
   if (data) {
     ...
     int32_t* ref_ptr = &(data->d_refcount);
     int32_t oldRefcount;
     do {
       oldRefcount = *ref_ptr;
       if(oldRefcount <= 0) { break; } /*only addRef is count >= 0! */
       newRefcount = oldRefcount + 1;
     } while(oldRefcount != CAS32(ref_ptr, oldRefcount, newRefcount));

#else /* CAS32 is not defined */
...
#endif /* CAS32 defined else clause */
  ...
  *_ex = 0;
  {
   ...
   if (data) {
     ...
     do {
       oldRefcount = *ref_ptr;
       if(oldRefcount <= 0) { 
         newRefcount = -1; /* don't self_destruct again */
         break;  /*only deleteRef if count >= 0! */
       }
       newRefcount = oldRefcount - 1;
     } while(oldRefcount != CAS32(ref_ptr, oldRefcount, newRefcount));

     self_destruct = (newRefcount == 0);

...
#define sidl_IsSuper_NUM 9
#define sidl_IsSuper_RETURN int
#define sidl_IsSuper_PROTO \
  (PyObject *obj)

#define sidl_MakeSuper_NUM 10
#define sidl_MakeSuper_RETURN void
#define sidl_MakeSuper_PROTO \
  (PyObject *obj)

#define sidl_API_pointers 11
...
static sidl_IsSuper_RETURN
sidl_IsSuper sidl_IsSuper_PROTO;

static sidl_MakeSuper_RETURN
...
/**
 * int
 * sidl_Object_Init(SPObject *sidlObject,
 *                  struct sidl_BaseInterface__object *ior,
 *                  int refType)
 *
...
#define sidl_IsSuper \
(*(sidl_IsSuper_RETURN (*)sidl_IsSuper_PROTO) \
sidl_Object_Adaptor_API[sidl_IsSuper_NUM])

#define sidl_MakeSuper \
(*(sidl_MakeSuper_RETURN (*)sidl_MakeSuper_PROTO) \
sidl_Object_Adaptor_API[sidl_MakeSuper_NUM])
...
{ \
  ...
  PyObject *_imp_module = NULL; \
  pthread_mutex_lock(&__sidl_pyapi_mutex); \
  _imp_module = PyImport_ImportModule("sidlObjA"); \
  if (_imp_module != NULL) { \
    PyObject *_imp_module_dict = PyModule_GetDict(_imp_module); \
    PyObject *_imp_c_api_object = PyDict_GetItemString(_imp_module_dict, "_C_API"); \
    if (PyCObject_Check(_imp_c_api_object)) { \
       sidl_Object_Adaptor_API = (void **)PyCObject_AsVoidPtr(_imp_c_api_object); \
    } \
...
#define import_SIDLObjA() \
{ \
  PyObject *_imp_module = PyImport_ImportModule("sidlObjA"); \
  if (_imp_module != NULL) { \
    PyObject *_imp_module_dict = PyModule_GetDict(_imp_module); \
    PyObject *_imp_c_api_object = PyDict_GetItemString(_imp_module_dict, "_C_API"); \
    if (PyCObject_Check(_imp_c_api_object)) { \

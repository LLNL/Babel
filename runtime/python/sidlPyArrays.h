...
{ \
  ...
  _imp_module = PyImport_ImportModule("sidlPyArrays"); \
  if (_imp_module != NULL) { \
    ...
    if (PyCObject_Check(_imp_c_api_object)) { \
       sidlPyArrays_API = (void **)PyCObject_AsVoidPtr(_imp_c_api_object); \
    } \
...
#define import_SIDLPyArrays() \
{ \
  PyObject *_imp_module = PyImport_ImportModule("sidlPyArrays"); \
  if (_imp_module != NULL) { \
    PyObject *_imp_module_dict = PyModule_GetDict(_imp_module); \
    PyObject *_imp_c_api_object = PyDict_GetItemString(_imp_module_dict, "_C_API"); \
    if (PyCObject_Check(_imp_c_api_object)) { \

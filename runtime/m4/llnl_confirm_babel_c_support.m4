...
dnl additional push required to apply change to CFLAGS
  ...
  AC_MSG_CHECKING([if C preprocessor supports BSD standard lib functions])	  
  AC_EGREP_HEADER([gmtime_r], [time.h], [AC_MSG_RESULT([yes])], 
    [ AC_MSG_RESULT([no])
      AC_MSG_ERROR([If this is icc, try to use icc -std=gnu99 instead of -c99])
    ])

  AC_MSG_CHECKING([for strdup])	  
  AC_EGREP_HEADER([strdup], [string.h], [AC_MSG_RESULT([yes])], 
    [ AC_MSG_RESULT([no])
      AC_MSG_ERROR([If this is icc, try to use icc -std=gnu99 instead of -c99])
    ])

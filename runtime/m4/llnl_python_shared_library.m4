  ...
  if test -n "$llnl_python_shared_library" -a -f "$llnl_python_shared_library" -a -n "$llnl_python_shared_library_dir" -a -d "$llnl_python_shared_library_dir" ; then
    ...
    case "$target_os" in
      cygwin*)
        ...
        ;;
      aix*)
        llnl_python_shared_library="libpython$llnl_cv_python_version$llnl_cv_python_abiflags.sl"
        ;;
      *)
        llnl_python_shared_library="libpython$llnl_cv_python_version$llnl_cv_python_abiflags$libltdl_cv_shlibext"
        ;;
    esac
    ...
    if test "$llnl_python_shared_library_found" != "yes"; then
      case "$target_os" in
        darwin*)
          llnl_python_shared_library="libpython$llnl_cv_python_version$llnl_cv_python_abiflags.dylib"
 	  llnl_ld_extra=
	  if test -f /etc/ld.so.conf; then
	    llnl_ld_extra=`$SED -e '/^#/d' -e '/^include/d' -e 's/[:,\t]/ /g;s/=[^=]*$//;s/=[^= ]* / /g' /etc/ld.so.conf 2>/dev/null | tr '\n' ' '`
	  fi

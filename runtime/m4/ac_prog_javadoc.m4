...
dnl @version $Id: ac_prog_javadoc.m4 5816 2007-01-29 21:45:13Z epperly $
dnl
...
if test "x$JAVAPREFIX" = x; then
  ...
  if test "x$JAVADOC" = x; then
    AC_PATH_PROG(JAVADOC, javadoc,,"${JAVAPREFIX}/bin:${JAVAPREFIX}:${PATH}")
  fi
fi
if test "x$JAVADOC" = x; then
  AC_MSG_ERROR([no acceptable javadoc generator found in \$PATH])
fi
AC_PROVIDE([$0])dnl
])

 ...
 */
static int 
setArrayDescStage1_Absoft(void* desc,
                          int rank,
 ...
 */
static int 
resetArrayDesc_Absoft(void* desc,
			  void* base_addr,
			  int rank,
			...
			)
{
  ...
  rc = setArrayDescStage1_Absoft(desc, rank, desc_type, data_type, element_size);
  if (rc) return rc;
  return resetArrayDesc_Absoft(desc, base_addr, rank,
...
getArrayDescSize_Absoft(int rank)
{
 ...
 */
static int 
nullifyArrayDesc_Absoft(void* desc, int rank)
{
  dope_vec* dv = (dope_vec*) desc;
  dv->assoc = 0;
  return 0;
...
/**
 * Set CompilerCharacteristics function pointers for Absoft.
 */
void F90_SetCCFunctions_Absoft(F90_CompilerCharacteristics* cc)
{
  cc->setArrayDesc              = setArrayDesc_Absoft;
  cc->getArrayDescSize	        = getArrayDescSize_Absoft;
  cc->nullifyArrayDesc          = nullifyArrayDesc_Absoft;
}
